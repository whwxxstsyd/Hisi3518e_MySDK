...
LOCAL_MODULE := $(patsubst $(LOCAL_PATH)/%,$(ROOTFS_OUT_DIR)/%,$(__FILES__))

# copy files
$(ROOTFS_OUT_DIR)/%: $(LOCAL_PATH)/%
	$(if $(wildcard $@),@rm -rf $@)
	@mkdir -p $(dir $@)
...
__LINKS__ := $(patsubst $(LOCAL_PATH)/%, $(ROOTFS_OUT_DIR)/%,$(__LINKS__))
...
$(foreach f,$(PRIVATE_FILES),
	$(if $(strip $(f)),
		$(if $(findstring .gitignore,$(f)),,
			@mkdir -p $(dir $(patsubst $(PRIVATE_PATH)/%,$(ROOTFS_OUT_DIR)/%,$(f)));
			@rm -rf $(patsubst $(PRIVATE_PATH)/%,$(ROOTFS_OUT_DIR)/%,$(f));
			@cp -dpRf $(f) $(patsubst $(PRIVATE_PATH)/%,$(ROOTFS_OUT_DIR)/%,$(f));
		)
	)
)
endef
...
$(foreach l,$(PRIVATE_LINKS),
	$(if $(strip $(l)),
		@mkdir -p $(dir $(l));
		cp -dpRf $(subst $(ROOTFS_OUT_DIR)/,$(PRIVATE_PATH)/,$(l)) $(l);
	)
)
endef
