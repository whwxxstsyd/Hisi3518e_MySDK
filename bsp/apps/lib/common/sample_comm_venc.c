...
HI_S32 SAMPLE_COMM_VENC_SizeH264(FILE* fpH264File, VENC_STREAM_S *pstStream)
{
    HI_S32 i,len=0;
            ...
            if(SAMPLE_RC_CBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_CIF:	
					   ...
					   stH264Cbr.fr32TargetFrmRate = 20;
                       break;
                    case PIC_D1:
                    ...
                    case PIC_HD720:   /* 1280 * 720 */
                       stH264Cbr.u32BitRate = 1024*1+512;
...
/* add by liwei */
#define     MAX_STREAM_CH       1
PUT_VIDEO_FRAME_CB put_frame_cb = NULL;
void *g_userdata = NULL;

void SAMPLE_COMM_VENC_SetStreamCB(PUT_VIDEO_FRAME_CB handle, void *user_data)
{
    g_userdata   = user_data;
    put_frame_cb = handle;//灏嗗畾涔夌殑put_frame_cb锛屾寚鍚慼andle鍑芥暟
}


void *SAMPLE_COMM_VENC_GetStreamProc(void *pArg)
{
    HI_S32 i,j;
    HI_S32 s32ChnTotal;
    VENC_CHN_ATTR_S stVencChnAttr;
	...
	HI_S32 frame_len = 0;
	HI_S32 my_frame_len = 0;
    HI_S8 data[HI_MAX_FRAME_SIZE*2] = {0};
    HI_U32 pts_ms = 0;
    HI_S32 *pIsRun = (HI_S32 *)pArg;
    ...
    ******************************************/
	...
	//while (*pIsRun)
    {
        ...
        else if (s32Ret == 0)
        {
            ...
            {
                frame_len = -1;
                SAMPLE_PRT("please restart init video device\n");                
                put_frame_cb(i, data, frame_len, pts_ms, g_userdata);
                return NULL;
            }
            continue;
        }
...
//                	SAMPLE_PRT("Select chn[%d]\n", i);
           ...
           /*******************************************************
                     step 2.4 : write frame to buffer
                    *******************************************************/
		  pData = data;
		  frame_len = HI_FRAME_HEAD_SIZE; 
		  pts_ms = (HI_U32)(stStream.pstPack[0].u64PTS / 1000); 
		  for(j = 0; j < stStream.u32PackCount; j++)
		  {
			 if (frame_len > sizeof(data))
			 {
				printf("!!!warn: h264_frame is too big\n");
				goto RELEASE_STREAM;
			 }
			 memcpy(pData + frame_len,stStream.pstPack[j].pu8Addr[0],stStream.pstPack[j].u32Len[0]);
		  	 frame_len += stStream.pstPack[j].u32Len[0];
		  }

		  //此时的data中保存了完整的一个pack数据包
		  //这里传入的data包括了前面的数据头部
		  if (put_frame_cb)
		  {
			put_frame_cb(i, data, frame_len, pts_ms, g_userdata);                   
...
RELEASE_STREAM:
                    ...
                    *******************************************************/
					...
					//SAMPLE_PRT("release stream finish\n")	;
                    /*******************************************************
                     step 2.6 : free pack nodes
                    *******************************************************/
                    free(stStream.pstPack);
                    stStream.pstPack = NULL;
					//SAMPLE_PRT("free pack nodes finish\n")	;
                }
            }
        }
    }
...
HI_S32 SAMPLE_COMM_VENC_GetSnapProc(VENC_CHN VencChn)
{
    struct timeval TimeoutVal;
    fd_set read_fds;
    HI_S32 s32VencFd;
    ...
    /* add by liwei */
    HI_S8 *pData;     
    HI_S32 frame_len = 0, i;
    HI_S8 data[HI_MAX_FRAME_SIZE * 5] = {0};
    HI_U32 pts_ms = 0;    
    ...
    else
    {
        if (FD_ISSET(s32VencFd, &read_fds))
        {
			...
			*******************************************************/
            ...
            pData = data;          /* 姝ゅ淇濈暀甯уご绌洪棿,閬垮厤鍦ㄤ笂灞傚湪鍋歮emcpy鎿嶄綔 */
            frame_len = HI_FRAME_HEAD_SIZE; 
            pts_ms = stStream.pstPack[0].u64PTS / 1000; /* 涓�甯ф暟鎹腑鎵�鏈夌殑鍖呯殑鏃堕棿鎴抽兘鐩稿悓 */
            for (i = 0; i < stStream.u32PackCount; i++)
            {      
                if (frame_len > sizeof(data))
                {
                    printf("!!!warn: jpg is too big\n");                    
                    goto RELEASE_STREAM;
                }
                memcpy(pData + frame_len, stStream.pstPack[i].pu8Addr + stStream.pstPack[i].u32Offset, 
                                stStream.pstPack[i].u32Len - stStream.pstPack[i].u32Offset);
                frame_len += stStream.pstPack[i].u32Len - stStream.pstPack[i].u32Offset;
            }
            if (put_frame_cb)                                      
                put_frame_cb(VencChn, data, frame_len, pts_ms, g_userdata);  
RELEASE_STREAM:            
            s32Ret = HI_MPI_VENC_ReleaseStream(VencChn, &stStream);
            if (s32Ret)
            {
