...
***********************************************************/
char * url_encode(char const *s, int len, unsigned char *to, int *new_length) 
{ 
...
void md5_auth_cvtHex( HASH bin, HASHHEX hex )
{
    unsigned short i;
    unsigned char j;
...
void md5_auth_digest_CalcHA1(const char* authId, const char* password,
                             ...
                             const char* cnonce, HASHHEX sessionKey )
{
    ...
    md5_auth_cvtHex( ha1, sessionKey );
}

void md5_auth_digest_CalcResponse( HASHHEX ha1, const char* nonce, const char* nc,
                                  ...
                                  const char* uri, HASHHEX entity, HASHHEX response )
{
    ...
    md5_auth_cvtHex( respHash, response );
}
                                 ...
                                 const char* method,const char*  uri)
{
    ...
    md5_auth_digest_CalcHA1(auth, pass, alg, realm, nonce, cnonce, ha1);
    md5_auth_digest_CalcResponse(ha1, nonce, nc, cnonce, qop, method, uri, ha2, response);
 ...
 */
void des_encrypt(uchar *key, uchar input[8], uchar output[8])
{
	des_context ctx;
	des_set_key(&ctx,  key);
 ...
 */
void des_decrypt(uchar *key, uchar input[8], uchar output[8] )
{
	des_context ctx;
	des_set_key(&ctx,  key);
...
int	Hex_2_Asc(unsigned char *pSrc, int len, unsigned char *pDest)
{
	...
	for(i = 0; i < len/2; i++, pSrc += 2)  
	{
		...
		if(Hex_2_Dec(*pSrc) < 0 || Hex_2_Dec(*(pSrc+1)) < 0)
			return -1;
		*(pDest+i) = (Hex_2_Dec(*pSrc) << 4 &0xf0) | (Hex_2_Dec(*(pSrc+1)) & 0xf);
	}
...
int Asc_2_Hex(unsigned char *pSrc, int len, unsigned char *pDest)
{
	...
	{
		*pDest = Dec_2_Hex((*pSrc & 0xf0) >> 4); //high four bits
		 pDest++;
		*pDest = Dec_2_Hex(*pSrc & 0x0f);        //lwo four bits
		 pDest++;
	}
	return (len*2);
}
