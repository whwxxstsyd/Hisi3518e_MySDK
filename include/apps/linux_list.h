...
#if defined(WIN32)		/* windows没有typeof关键字,需要传入父结构体的类型 */ 
/**
 ...
 * @member:	the name of the list_struct within the struct.
 */
#define list_for_each_entry(pos, head, member, typeof)				\
	for (pos = list_entry((head)->next, typeof, member);	\
	     &pos->member != (head); 					\
		 pos = list_entry(pos->member.next, typeof, member))
 ...
 * @member:	the name of the list_struct within the struct.
 */
#define list_for_each_entry_safe(pos, n, head, member, typeof)			\
	for (pos = list_entry((head)->next, typeof, member),	\
	n = list_entry(pos->member.next, typeof, member);	\
	     &pos->member != (head); 					\
		 pos = n, n = list_entry(n->member.next, typeof, member))
#else		/* linux版本 */
/**
 * list_for_each_entry	-	iterate over list of given type
 * @pos:	the type * to use as a loop counter.
...
#define list_for_each_entry_safe_ext(pos, n, head, member)			\
