...
/* add by liwei */
#define     HI_MAX_FRAME_SIZE         (512*1024)   /* 与common.h中的MAX_FRAME_LEN相同 */

typedef struct HI_FRAME_HEAD_T
{
    char mark[4];          /* "TNVS" */
    unsigned int pts;
...
#define     HI_FRAME_HEAD_SIZE          (sizeof(HI_FRAME_HEAD))           /* 与common.h中的FRAME_HEAD_LEN相同 */
typedef void(*PUT_VIDEO_FRAME_CB)(int ch, char *data, int len, unsigned int pts, void *user_data);
/* add by liwei for md */
//  typedef void(*ON_DEAL_RAWDATA_CB)(VDA_MB_SAD_DATA_S *stMbSadData);
//  typedef void(*ON_DEAL_RESULT_CB)(VDA_OBJ_DATA_S *stObjData);

//	void SAMPLE_SET_RAWDATA_CB(ON_DEAL_RAWDATA_CB OnDealRawData);
//	void SAMPLE_SET_RESULT_CB(ON_DEAL_RESULT_CB OnDealResult);
/* end add */
