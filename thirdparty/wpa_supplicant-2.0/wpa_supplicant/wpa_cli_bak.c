...
static int wpa_cli_open_connection(const char *ifname, int attach)
{
	...
	printf("CONFIG_CTRL_IFACE_NAMED_PIPE\n");
	...
	printf("CONFIG_CTRL_IFACE_NAMED_PIPE_2\n");
	char *cfile = NULL;
	int flen, res;
...
int my_wpa_ctrl_command(struct wpa_ctrl *ctrl, const char *cmd, char *buf, int *len)
{
	...
	if ((ctrl == NULL) || (cmd == NULL) || (buf == NULL) || (len == NULL))
		return -1;
	...
	if (ifname_prefix) {        /* ifname_prefix = NULL, ignore this */
		os_snprintf(buf, *len, "IFNAME=%s %s",
			    ifname_prefix, cmd);
		buf[sizeof(buf) - 1] = '\0';
		cmd = buf;
	}	  
	ret = wpa_ctrl_request(ctrl, cmd, os_strlen(cmd), buf, 
								(size_t *)len, wpa_cli_msg_cb);
	if (ret == -2) {
		printf("'%s' command timed out.\n", cmd);
		return -2;
	} else if (ret < 0) {
...
struct wpa_ctrl *wpa_init(void)
{
	...
	if (eloop_init())
		return NULL;
//	eloop_register_signal_terminate(wpa_cli_terminate, NULL);		modify by liwei
	if (ctrl_ifname == NULL)
		ctrl_ifname = wpa_cli_get_default_ifname();
	if (wpa_cli_open_connection(ctrl_ifname, 0) < 0)
		return NULL;
	if (ctrl_conn == NULL)
		printf("wpa_cli_open_connection error\n"); 
